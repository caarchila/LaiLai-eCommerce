...
useEffect(()=>{
  ...
  var select = document.getElementById(`dropdown-detalle-${obj.idOpcion}`);
    select.selectedIndex = 0;
    agregarValores(select.value);
}else{
  if(carritoSeleccionado.opciones.length > 0){
    carritoSeleccionado.opciones.map(
      o => {
        o.varianteSeleccionada.map(
          v =>{
            obj.variantes.map((vObj,index)=>{
              if(vObj.idVariante === v.idVariante){
                    let select = document.getElementById(`dropdown-detalle-${o.idOpcion+''+edi}`);
...
const agregarValores = (id) =>{
  const variante = obj.variantes.filter(v => parseInt(v.idVariante) === parseInt(id))[0];
