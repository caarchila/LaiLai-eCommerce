...
class CarritoCompras extends React.Component {
    ...
    if (Object.keys(this.props.user).length > 0) {
      if (this.props.cart.length > 0) {
        ...
        const myGrowl = await growl({
          ...
          title: "información",
          message:
            "No puede procesar la compra porque no tiene productos en el stock.",
        });
      }
    } else {
      this.setState(
  ...
  render() {
    ...
    const total = carrito.reduce(function (suma, c) {
    ...
    return (
      <>
        <Modal
          size="lg"
          dialogClassName="contenedor-dialog-carrito"
          show={this.props.modal}
          onHide={this.handleClose}
          contentClassName="contenedorCarritoCompras"
        >
          <div className="headerCarritoCompras text-left">
            <h3>
              <strong>Orden</strong>
            </h3>
            <button className="cerrarCarrito" onClick={this.handleClose}>
              {" "}
              <MDBIcon icon="times" />
            </button>
          </div>
          <div className="contenedorOrdenes">
            {carrito.map((c) => (
              <ProductoCarritoCompras
                key={c.id}
                carrito={c}
                levantarIdProducto={this.getIdProducto}
              />
            ))}
          </div>
          <Row>
            <Col sm={6} md={6} xl={6}>
              <h4>
                {" "}
                <strong>Total Q {parseFloat(total).toFixed(2)}</strong>
              </h4>
              <Button id="btnProcesarOrden" onClick={this.procesarCompra}>
                Procesar mi orden
              </Button>
            </Col>
            <Col sm={6} md={6} xl={6}>
        ...
        <ModalEditarProducto
          ...
          detalleMenu={detalle}
          detalleSelected={productoSelected}
        />
