  ...
  render(){
    return(
      <Alert variant={"secondary"} className="d-flex justify-content-between my-1 contenedor-pro-carrito-comp" onClick={(e) => {this.getIdProducto(e, this.props.carrito)}}>
        <Badge variant={"dark"} className="cantidadCarritoCompras"><span className='tamaño'>{this.props.carrito.cantidad}</span></Badge>
        ...
        <Badge variant="warning" className="cantidadCarritoCompras"><span className='tamaño'>{parseFloat(this.props.carrito.precioFinal).toFixed(2)}</span></Badge>
