...
class HeaderMenu extends React.Component {
  handleReloadMenus(id, subcat, index) {
    if (this.props.handleUpdateMenu !== undefined) {
      if (subcat !== "Si") {
        this.props.handleUpdateMenu(
          this.props.categorias.filter((c) => parseInt(c.id) === parseInt(id)),
        );
      } else {
        this.props.handleUpdateMenu(
          this.props.categorias[index].subcategorias.filter(
        ...
        {categorias.map((c, index) =>
          c.subcategoria !== "Si" ? (
            <Nav.Item
              key={c.id}
              className={
                parseInt(c.id) === parseInt(this.props.id)
                  ? "activeMenu contenedorLinks"
                  : "contenedorLinks"
              }
            >
              <Link
                ...
                to={`/categoria/${c.id}/${0}`}
                onClick={() => {
                  this.handleReloadMenus(c.id);
                  this.handleCambiarEstado();
                }}
              >
                {c.nombre}
              </Link>
            ...
            <NavDropdown title={c.nombre} key={c.id}>
              {c.subcategorias.map((s) => (
                <Link
                  ...
                  to={`/categoria/${c.id}/${s.id}`}
                  onClick={() => {
                    this.handleReloadMenus(s.id, c.subcategoria, index);
                    this.handleCambiarEstado();
                  }}
                >
                  {s.nombre}
