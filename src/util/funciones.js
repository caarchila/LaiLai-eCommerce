...
export function validarHorarioSeleccionado(
  ...
  if (horaApertura !== undefined) {
    ...
    if (fechaProgramadaFin.toString() === fechaActual.toString()) {
      ...
      if (
        horaProgramadaDate >= horaInicio &&
        horaProgramadaDate <= HoraFin &&
        horaProgramadaDate > horaActual
      ) {
        ...
        if (horaProgramadaDate > horaActual && horaProgramadaDate < HoraFin) {
          ...
          respuesta.msj =
            "La hora especificada debe tener por lo menos 45 minutos despues de la hora actual y 90 antes de la hora de cierre";
        }
      } else {
        respuesta.msj =
      ...
      if (horaProgramadaDate >= horaInicio && horaProgramadaDate <= HoraFin) {
        ...
        if (horaProgramadaDate > horaInicio && horaProgramadaDate < HoraFin) {
          ...
          respuesta.msj =
            "La hora especificada debe tener por lo menos 45 minutos despues de la hora de apertura y 90 antes de la hora de cierre";
        }
      } else {
        respuesta.msj =
          "La hora especificada esta fuera de los horarios de la tienda";
