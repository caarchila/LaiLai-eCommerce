        ...
        <Row>
          {esSubCategoria === false ? (
            menus.length === 0 ? (
              ...
              menus[0].subcategorias.map((mc) => (
                <CardsMenu
                  key={mc.id}
                  id={match.params.id}
                  idSub={mc.id}
                  imagen={mc.imagenes.normal}
                  unavailable={mc.imagenes.unavailable}
                  titulo={mc.nombre}
                  cambiarEstado={cambiarEsSubCategoria}
                />
              ))
            )
          ) : menusSubCategorias.length === 0 ? (
            <Spinner animation="spinner" variant="primary" />
          ) : (
            menusSubCategorias.map((mc) => (
              <CardsMenu
                key={mc.id}
                id={match.params.id}
                idSub={mc.id}
